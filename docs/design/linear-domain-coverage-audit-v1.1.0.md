...
> **Status**: Completed design artifact for [M5-D1](../implementation-plan.md#m5-d1---linear-domain-coverage-audit-and-adapter-boundary--v110)
...
| Traversal and rendered relations | Read visible issue relations for traversal and persisted frontmatter | `Issue.get_links()` / `await issue.links` | Covered by domain layer | `get_ticket_relations(issue_ids)` remains a gateway helper because traversal wants a batch-shaped mapping, but its approved implementation path is domain-layer fan-out over `Issue.get_links()`, not local raw GraphQL. `Issue.get_links()` covers issue-link relations only, is capped at 250 forward plus 250 inverse links per issue, and must be normalized into local `blocks`, `is_blocked_by`, and `relates_to` dimensions without inventing `parent` / `child` from issue-link data. |
...
| Refresh comment cursor | Batch-read comment/thread metadata for `comments_signature` across the tracked reachable set | Per-issue `Comment.updated_at`, `resolved_at`, `parent`, and `children` now exist, but no batch-by-issue-set surface exists | Partially covered | Per-issue fetches move to the domain layer; this supersedes the earlier [M3-O1](../implementation-plan.md#m3-o1---comments-signature-input-settlement) raw-only conclusion for single-ticket fetches. The batched refresh path still needs `get_refresh_comment_metadata(issue_ids)`. |
...
This supersedes the earlier [M3-O1](../implementation-plan.md#m3-o1---comments-signature-input-settlement) raw-adapter-only conclusion for per-issue comment reads. [M3-O1](../execution/M3-O1.md) remains authoritative for the accepted `comments_signature` input on the batched refresh path.

The real gateway should therefore follow these rules:

- `fetch_issue(issue_id_or_key)` resolves the issue through the domain layer, catches `linear_client.LinearNotFoundError` on explicit not-found paths and maps it to `RootNotFoundError`, reads comments through `Issue.get_comments()` and `Comment.children`, reads attachments through `Issue.get_attachments()`, reads per-issue relations through `Issue.get_links()`, and fills the remaining bundle fields through the dedicated workspace-identity and label helpers. If `Issue.get_comments()` returns synthetic placeholder parents, exclude those placeholders from the persisted bundle and promote each visible child of a placeholder to its own top-level thread root (`parent_comment_id=None`) so rendering and local cursor derivation operate on real comments only.
- `get_ticket_relations(issue_ids)` remains a gateway helper because traversal wants a mapping keyed by issue UUID, but its approved implementation path is domain-layer fan-out over `Issue.get_links()`. The gateway implementation should deduplicate input ids, own bounded concurrency internally, and return normalized `RelationData` values to callers. The packaged surface is accepted as authoritative up to its built-in cap of 250 forward links and 250 inverse links per issue. Do not add a raw fallback for that cap without a separate accepted amendment.
...
Use these normalization rules when converting `IssueLink` values into `RelationData` for one fetched issue:

| `IssueLink` perspective relative to the fetched issue | Upstream `link_type` | Emit `RelationData.dimension` | Emit `RelationData.relation_type` | Target endpoint |
| --- | --- | --- | --- | --- |
| `from_issue.id == current_issue_id` | `"blocks"` | `blocks` | `"blocks"` | `to_issue` |
| `to_issue.id == current_issue_id` | `"blocks"` | `is_blocked_by` | `"blocks"` | `from_issue` |
| Either direction | `"related"`, `"duplicate"`, `"similar"` | `relates_to` | Preserve the raw upstream value | The opposite endpoint |
| Either direction | Any other value | Not part of the current audited mapping | Preserve the raw upstream value only if a later accepted amendment adds a local dimension | The opposite endpoint |

The local `parent` / `child` hierarchy dimensions are not `IssueLink` conversions. The gateway must not invent those dimensions from issue-link data.

Raw `linear.gql.*` usage is approved only for these five helper categories:

- `get_workspace_identity(issue_id)` — stable workspace UUID plus slug for manifest validation
...
All raw helpers are read-only helpers inside the real gateway. They must use `query(...)` or `paginate_connection(...)` as appropriate, own pagination internally, return fully materialized results, and never call `gql(...)` or `mutate(...)`. At the gateway error boundary, catch `linear_client.LinearNotFoundError` on explicit entity-lookups and map it to `RootNotFoundError`; map upstream auth, transport, or GraphQL failures that make the current run non-viable to `SystemicRemoteError`. If later implementation work shows that traversal relation fan-out materially needs a batched raw relation-read helper for correctness or cost, treat that as a separate plan amendment rather than silently widening this boundary.
...
This section names protocol-level boundary positions. The real gateway may use differently named private helper functions internally, but the public boundary name for workspace validation is `get_workspace_identity(issue_id)`.
...
That means the historical [M3-O1](../execution/M3-O1.md) statement that per-issue comment metadata was unavailable through the domain layer is now superseded for single-ticket fetches. Its accepted raw-metadata input remains relevant only to `get_refresh_comment_metadata(issue_ids)`.
...
- Keeping `get_ticket_relations(issue_ids)` on domain-layer fan-out keeps the boundary narrow and more faithful to the packaged surface, but it also accepts the packaged `Issue.get_links()` cap of 250 forward and 250 inverse links per issue as the current v1 operational limit for single-ticket relation reads. Do not widen the raw boundary pre-emptively; if maintained evidence later shows tickets exceeding that cap or proves that a raw fallback is needed for completeness rather than performance, represent that as a separate accepted amendment.
- The current [src/context_sync/_gateway.py](../../src/context_sync/_gateway.py) protocol docstrings still describe `get_ticket_relations(issue_ids)` as a raw-GraphQL path and still allow `get_refresh_relation_metadata(issue_ids)` to share that underlying query. [M5-1](../implementation-plan.md#m5-1---real-linear-gateway-and-runtime-wiring) must update those docstrings while implementing the audited boundary so the code comments stop contradicting this design artifact.
