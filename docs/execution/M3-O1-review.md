...
| --- | --- | --- |
| [M3-O1-R1](M3-O1-review.md#findings) | Fix now | Agreed. The deletion mapping was accepted without probe evidence. Ran a supplementary deletion probe (ARK-18): `commentDelete` is a hard delete — the comment is removed from `issue.comments` and from direct `comment(id:)` queries, and `includeArchived: true` does not recover it. There is no `commentArchive` mutation. Updated [M3-O1.md](M3-O1.md) with the probe evidence (Probe 4 in behavioral probes), corrected the deletion signal mapping to reflect the observed behavior (`unknown` for absent previously-tracked comments, `true` for non-null `archivedAt`), and added the probe to the verification section. |
| [M3-O1-R2](M3-O1-review.md#findings) | Fix now | Agreed. Updated [docs/design/linear-domain-coverage-audit.md](../design/linear-domain-coverage-audit.md) Section 5 to mark the gate as resolved, reference the accepted [M3-O1](M3-O1.md) outcome, and direct readers to the full probe evidence. The section now reads as resolved rather than open. |
