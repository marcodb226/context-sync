...
| --- | --- | --- |
| M1-1-R1 | Fix now | Valid. The fake harness should model invisible issues so [M3-1](../implementation-plan.md#m3-1---incremental-refresh-and-quarantined-root-recovery) can test quarantine without ad-hoc mocks. Adding a `hide_issue()` method and updating `get_refresh_issue_metadata` to return `visible=False` for hidden IDs, plus a test. |
| M1-1-R2 | Fix now | Valid bug. The fake conflates `parent_comment_id` with `root_comment_id` for nested replies. Fixing by using the `CommentData` to walk the parent chain within the bundle, and adding a multi-level reply test. |
| M1-1-R3 | Fix now | Valid. The link points at a non-existent `docs/planning/implementation-plan.md`. Correcting to [docs/implementation-plan.md](../implementation-plan.md). |
| M1-1-R4 | Fix now | Valid type mismatch. Aligning all four fake batch methods to `Sequence[str]` to match the `LinearGateway` Protocol. |
| M1-1-R5 | Fix now | The constructor-level default with per-call override is intentional — it avoids forcing callers to repeat the cap on every `sync()` call. Updating [docs/design/0-top-level-design.md](../design/0-top-level-design.md#1-library-api) to reflect the constructor-level pattern and recording the decision in [M1-1.md](M1-1.md). |
| M1-1-R6 | Fix now | Valid. Adding `@runtime_checkable` to `LinearGateway` and switching the test to `assert isinstance(gw, LinearGateway)` for a genuine runtime conformance check. |
