...
| --- | --- | --- |
| M4-2-R1 | Fix now | The reviewer is right that the docs overstate runtime readiness. The real gateway adapter is out of M4-2 scope (no ticket exists for it yet), but the README should not present an install-and-run experience that silently fails. Add a pre-release notice to [README.md](../../README.md) making the current state explicit. |
| M4-2-R2 | Fix now | The tests should be reclassified honestly. Rename the module and docstring to reflect what they actually test (library pipeline via private handlers, not full CLI dispatch). True `main()`-level integration testing is blocked on the real gateway and should be tracked separately. |
| M4-2-R3 | Defer to [M5-2](../implementation-plan.md#m5-2---supported-public-runtime-validation-and-smoke-path) | A manual smoke-test procedure requires a working CLI against real Linear, which is blocked on the real gateway adapter. That deferred obligation is now tracked explicitly by [M5-2](../implementation-plan.md#m5-2---supported-public-runtime-validation-and-smoke-path), which owns the supported public-entrypoint validation path plus the durable smoke recipe for one successful and one representative failure run. |
| M4-2-R4 | Fix now | Rewrite the [tests/test_e2e.py](../../tests/test_e2e.py) docstring to accurately describe the scope: library pipeline tests via private CLI handlers with a fake gateway, not full CLI-to-output proof. |
| M4-2-R5 | Fix now | Import `DEFAULT_MAX_TICKETS_PER_ROOT` from [src/context_sync/_config.py](../../src/context_sync/_config.py) and `DEFAULT_LOG_LEVEL` from [src/context_sync/_cli.py](../../src/context_sync/_cli.py) in the test helper to eliminate the duplicated literals. |
| M4-2-R6 | Fix now | Add an INFO-level abort log in the `finally` path of each public entry point (`sync`, `refresh`, `add`, `remove_root`, `diff`) so a "started" log always has a matching "completed" or "aborted" event. |
| M4-2-R7 | Fix now | Standardize on `active_roots=` across all modes. The sync started log currently uses `root_count=`; change it to match `active_roots=` used by the refresh path. |
| M4-2-R8 | Fix now | Expand the README logging section to distinguish mutating modes from `diff`, documenting diff's actual field names (`tracked_tickets`, `current`, `stale`, `missing_locally`, `missing_remotely`). |
| M4-2-R9 | Fix now | Rename `errored_uids` to `errored_keys` in [src/context_sync/_diff.py](../../src/context_sync/_diff.py) to match the actual content semantics. |
