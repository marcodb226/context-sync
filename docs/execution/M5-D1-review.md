...
| --- | --- | --- |
| M5-D1-R1 | Fix now | Agreed. [docs/design/linear-domain-coverage-audit-v1.1.0.md](../design/linear-domain-coverage-audit-v1.1.0.md) now records the stale [src/context_sync/_gateway.py](../../src/context_sync/_gateway.py) relation docstrings as an explicit implementation delta, and [M5-1](../implementation-plan.md#m5-1---real-linear-gateway-and-runtime-wiring) now carries the same docstring-update requirement so the code comments can no longer drift silently from the audited boundary. |
| M5-D1-R2 | Fix now | Agreed. Standardized the public boundary name to `get_workspace_identity(issue_id)` across the audit and added a clarifying sentence in the type-mapping section that private helper names inside the real gateway may differ, but the protocol-level boundary name is `get_workspace_identity(issue_id)`. |
| M5-D1-R3 | Fix now | Agreed. [docs/design/linear-domain-coverage-audit-v1.1.0.md](../design/linear-domain-coverage-audit-v1.1.0.md) now explicitly says that v1.1.0 supersedes the earlier [M3-O1](../implementation-plan.md#m3-o1---comments-signature-input-settlement) raw-only conclusion for per-issue comment reads while preserving [M3-O1](M3-O1.md) as the authoritative accepted input for the batched refresh path. |
| M5-D1-R4 | Fix now | Agreed. The audit boundary decision now records the `linear_client.LinearNotFoundError` mapping to the gateway error contract, and [M5-1](../implementation-plan.md#m5-1---real-linear-gateway-and-runtime-wiring) now carries the same explicit exception-mapping requirement. |
| M5-D1-R5 | Fix now | Agreed. The audit now makes the packaged `Issue.get_links()` cap explicit, accepts that 250-forward / 250-inverse bound as the current v1 operational limit for the domain-layer path, and routes any future raw fallback widening through a separate accepted amendment rather than leaving the contract implicit. |
| M5-D1-R6 | Fix now | Agreed. The audit now includes a normalization table for `IssueLink` → `RelationData`, defining the fetched-issue perspective, the `blocks` / `is_blocked_by` inversion rule, the informational `relates_to` mapping, the preserved raw `relation_type`, and the explicit non-mapping for `parent` / `child`. |
| M5-D1-R7 | Fix now | Agreed. The audit now defines placeholder-comment handling during flattening: synthetic `Comment.is_placeholder()` parents stay out of persisted bundles, and each visible child of a placeholder is normalized into its own top-level thread root. [M5-1](../implementation-plan.md#m5-1---real-linear-gateway-and-runtime-wiring) now carries the same rule so bundle rendering and local `comments_signature` derivation stay aligned. |
